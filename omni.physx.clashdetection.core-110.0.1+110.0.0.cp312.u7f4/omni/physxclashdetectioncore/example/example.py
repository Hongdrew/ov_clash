...
class ClashDetectionProcessor:
    ...
    def _clean_overlaps_and_query(self) -> bool:
        ...
        num_affected_records = clash_data.remove_all_overlaps_by_query_id(self._query.identifier, False)
        print(f"{num_affected_records} clash {'record' if num_affected_records == 1 else 'records'} removed.")
        num_affected_records = clash_data.remove_query_by_id(self._query.identifier)
