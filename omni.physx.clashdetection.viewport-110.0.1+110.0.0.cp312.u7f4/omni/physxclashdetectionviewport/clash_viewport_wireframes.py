...
__all__ = ["ClashViewportWireframes"]


class ClashViewportWireframes:
    """Helper class for creating and processing wireframe meshes with offset to avoid z-fighting."""

    @staticmethod
    def clone_mesh_with_offset(
        ...
        """Clone a source UsdGeom.Mesh with a small offset to avoid z-fighting.

        This method clones the mesh and offsets all vertices along their normals by a small epsilon value.
        ...
        - destination_mesh (UsdGeom.Mesh): A mesh that will receive offset clone of source_mesh
        ...
        """
        # Get mesh data
        points = np.array(source_mesh.GetPointsAttr().Get(usd_timecode))
        face_vertex_indices = source_mesh.GetFaceVertexIndicesAttr().Get(usd_timecode)
        face_vertex_counts = source_mesh.GetFaceVertexCountsAttr().Get(usd_timecode)

        # Get or compute normals
        normals_attr = source_mesh.GetNormalsAttr()
        normals = None
        normals_interpolation = None

        if normals_attr:
            normals_data = normals_attr.Get(usd_timecode)
            if normals_data is not None and len(normals_data) > 0:
                normals = np.array(normals_data)
                normals_interpolation = source_mesh.GetNormalsInterpolation()

        # If normals are not available or are not per-vertex, compute vertex normals
        if normals is None or normals_interpolation != UsdGeom.Tokens.vertex or len(normals) != len(points):
            normals = ClashViewportWireframes._compute_vertex_normals_optimized(
                points, face_vertex_indices, face_vertex_counts, visible_face_indices
            )
        ...
        # Set mesh attributes
        destination_mesh.GetPointsAttr().Set(offset_points)
        destination_mesh.GetFaceVertexIndicesAttr().Set(face_vertex_indices)
        destination_mesh.GetFaceVertexCountsAttr().Set(face_vertex_counts)

        # Update extent to account for offset
        try:
            extent = source_mesh.GetExtentAttr().Get(usd_timecode)
            if extent:
                # Expand extent slightly to account for offset
                extent_min = Gf.Vec3f(extent[0]) - Gf.Vec3f(epsilon, epsilon, epsilon)
                extent_max = Gf.Vec3f(extent[1]) + Gf.Vec3f(epsilon, epsilon, epsilon)
                destination_mesh.GetExtentAttr().Set([extent_min, extent_max])
        except Exception:
            pass
    ...
    def _compute_vertex_normals_optimized(
        ...
        if np.any(triangle_mask):
            with CodeTimer("_compute_normals_triangles_vectorized"):
                ClashViewportWireframes._process_triangles_vectorized(
        ...
        if np.any(quad_mask):
            with CodeTimer("_compute_normals_quads_vectorized"):
                ClashViewportWireframes._process_quads_vectorized(
        ...
        if np.any(polygon_mask):
            with CodeTimer("_compute_normals_polygons"):
                ClashViewportWireframes._process_polygons(
